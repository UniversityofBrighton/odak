...
{% for mySession in sessions %}
	<div class="session-content" id="{{mySession.session_id}}" >
		...
		<h3 style="overflow: auto;"> <a href="skskks{{mySession.permalink}}" {% if mySession.workshopId == 1%}style="color: #602f6b;"{% endif %}><span style="float: left; margin-top: 10px; margin-right: 10px;">{{mySession.title}}</span></a>{% if mySession.youtube_url%}<a href="{{mySession.youtube_url}}" target="_blank"><img src="/img/program/yt.jpg" height="25px" width="40px" style="float: left;  margin-top: 10px;"></a>{% endif %} {% if mySession.abstract %}<a href="{{mySession.rc_link}}" ></a>{% endif %}<a href="#intro" ><img src="/odak/img/program/back-to-top.jpg" height="40px" width="65px" style="float: right;"></a></h3>
